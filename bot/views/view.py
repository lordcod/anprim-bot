...
from bot.misc.env import (
    channel_suggest, channel_suggest_accept, accept_roles)
from bot import db
...
class ConfirmModal(nextcord.ui.Modal):
    ...
    async def callback(self, inter: nextcord.Interaction) -> None:
        ...
        embed.set_footer(
            text=f'Одобрено | {name}', icon_url=interaction.user.display_avatar)
        ...
        embed_accept.add_field(
            name='Суть идеи:', value=embed.fields[0].value, inline=False)
        if val:
            embed_accept.add_field(
                name='Аргумент подтверждения:', value=val, inline=False)
        embed_accept.set_footer(
            text=name, icon_url=interaction.user.display_avatar)
...
class Confirm(nextcord.ui.View):
    ...
    async def cancel(self, button: nextcord.ui.Button, interaction: nextcord.Interaction):
        ...
        embed.set_footer(
            text=f'Отказано | {name}', icon_url=interaction.user.display_avatar)
